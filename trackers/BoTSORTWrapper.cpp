...
BoTSORTWrapper::BoTSORTWrapper(const TrackConfig &config) : BaseTracker(config) {
    // Initialize BoTSORT with configuration paths
    tracker_ = std::make_unique<botsort::BoTSORT>(config.tracker_config_path, config.gmc_config_path,
                                                  config.reid_config_path, config.reid_onnx_path);
}

BoTSORTWrapper::~BoTSORTWrapper() = default;

std::vector<TrackedObject> BoTSORTWrapper::update(const std::vector<neuriplo_tasks::Detection> &detections,
                                                  const cv::Mat &frame) {
    ...
    // Update tracker
    std::vector<std::shared_ptr<botsort::Track>> tracks = tracker_->track(detectionsToTrack, frame);
