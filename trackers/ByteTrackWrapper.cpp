...
ByteTrackWrapper::ByteTrackWrapper(const TrackConfig &config) : BaseTracker(config) {
    ...
    tracker_ = std::make_unique<byte_track::BYTETracker>(30, // frame_rate
...
std::vector<TrackedObject> ByteTrackWrapper::update(const std::vector<neuriplo_tasks::Detection> &detections,
                                                    const cv::Mat &frame) {
    ...
    for (const auto &detection : detections) {
        if (config_.classes_to_track.find(static_cast<int>(detection.class_id)) != config_.classes_to_track.end()) {
            byte_track::Rect<float> rect(detection.bbox.x, detection.bbox.y, detection.bbox.width,
                                         detection.bbox.height);
            byte_track::Object obj(rect, static_cast<int>(detection.class_id), detection.class_confidence);
            objects.push_back(obj);
        }
    }

    // Update tracker
    std::vector<std::shared_ptr<byte_track::STrack>> tracks = tracker_->update(objects);
